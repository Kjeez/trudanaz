...
export default function MobileNav({ links }: { links: NavLink[] }) {
  ...
  return (
    <div className="md:hidden">
      ...
      {open && (
        <div
          className="fixed top-0 left-0 z-[55] w-[100vw] h-[100dvh] bg-ink/60 backdrop-blur-md"
        ...
        <div className="flex flex-col pt-24 px-6 gap-2 h-[100dvh]" style={{ backgroundColor: "#ffffff" }}>
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`text-[15px] font-bold px-4 py-3.5 rounded-2xl transition-all ${
                pathname === link.href
          ...
          <a
            ...
            className="flex items-center justify-center gap-2 bg-whatsapp hover:bg-whatsapp-hover text-white text-sm font-bold px-5 py-3.5 rounded-full transition-colors shadow-md shadow-whatsapp/20 mt-2"
          >
